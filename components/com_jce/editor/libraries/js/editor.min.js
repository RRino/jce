...
(function (tinymce) {
    ...
    var WfEditor = {
        ...
        init: function (settings) {
            ...
            this.settings = this.settings || tinymce.extend({
        ...
        markLoaded: function () {
            ...
            each(['core', 'help', 'autolink', 'cleanup', 'code', 'format', 'importcss', 'colorpicker', 'upload', 'figure', 'ui', 'noneditable', 'branding'], function (n) {
                load('plugins/' + n + '/editor_plugin' + suffix + '.js');
            });
        },
        ...
        load: function () {
            ...
            if (Joomla.getOptions) {
                s = tinymce.extend(s, Joomla.getOptions('plg_editor_jce', {}));
            }
            ...
            tinymce.onAddEditor.add(function (mgr, ed) {
                ...
                if (Joomla.editors) {
                    Joomla.editors.instances[ed.id] = {
                        ...
                        getSelection: function () {
                            return self.getSelection(ed.id, { format: 'text' });
                        },
                ...
                if (s.compress.css) {
                    ed.onPreInit.add(function () {
                        ed.dom.loadCSS(s.site_url + 'index.php?option=com_jce&task=editor.pack&type=css&layout=content');
                    });
                }
                ...
                ed.onBeforeRenderUI.add(function () {
                    ...
                    if (!n.submit.nodeType && !n.submit.length) {
                        ...
                        n.submit = function () {
                            ...
                            return ed.formElement.mceOldSubmit(ed.formElement);
                        };
                        n = null;
                    }
                });
        ...
        initSubformEvents: function () {
            ...
            function createEditor(el) {
                var state = true, ed = tinymce.EditorManager.get(el.id);

                // editor does not exist
                if (!ed) {
                    state = self.createInstance(el);
            ...
            if (window.jQuery) {
                ...
                jQuery(document).on('sortstop', 'div.subform-repeatable', function (e, ui) {
                    if (ui.item[0]) {
        ...
        createInstance: function (el) {
            ...
            if (s.use_state_cookies !== false) {
                state = sessionStorage.getItem('wf_editor_state_' + el.id);
            }
        ...
        preinit: function (elements) {
            ...
            try {
                ...
                each(elements, function (elm) {
                    ...
                    var state = true, editorId = createId(elm);
        ...
        toggleEditor: function (el) {
            ...
            if (!ed) {
                ...
                if (this.settings.use_state_cookies !== false) {
                    sessionStorage.setItem('wf_editor_state_' + el.id, 1);
                }
                ...
                if (DOM.hasClass(el, 'wf-no-editor')) {
                    ...
                    // store state
                    if (this.settings.use_state_cookies !== false) {
                        sessionStorage.setItem('wf_editor_state_' + el.id, 0);
                    }
        ...
        getEditor: function (el) {
            ...
            if (el) {
                ...
                if (el && el.id) {
                    ed = win.wfe.get(el.id);
                }
            }

            // get active editor
            if (!ed) {
                ed = win.wfe.activeEditor;
            }
        ...
        toggleTab: function (ed, tab) {
            ...
            if (ed.activeLine && ed === tinymce.activeEditor) {
                window.setTimeout(function () {
        ...
        createTabs: function (ed) {
            ...
            // create it if it doesn't exist (K2, WidgetKit etc.)
            if (!header) {
                header = this.createHeader(ed.getElement());
            }
            ...
            // wrap in "joomla-tab" custom element for J4 compatability...
            header = DOM.add(header, 'joomla-tab', { 'orientation': 'horizontal', 'view': 'tabs' });

            var tablist = DOM.add(header, 'div', { 'role': 'tablist' });

            var ul = DOM.add(tablist, 'ul', {
            ...
            function makeTabActive(tab) {
                ...
                DOM.addClass(DOM.select('a', tab), 'active');
                DOM.setAttrib(DOM.select('button', tab), 'aria-expanded', 'true');
            }

            // Editor tab
            var editorTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-wysiwyg" aria-expaned="false" class="btn-link nav-link" role="tab">' + ed.getLang('tab.wysiwyg', 'Editor') + '</button></a>');

            // Source tab
            if (tinymce.inArray(plugins, 'source') !== -1) {
                var sourceTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-source" aria-expaned="false" class="btn-link nav-link" role="tab">' + ed.getLang('tab.code', 'Code') + '</button></a>');
            ...
            if (tinymce.inArray(plugins, 'preview') !== -1) {
                var previewTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-preview" aria-expaned="false" class="btn-link nav-link" role="tab">' + ed.getLang('tab.preview', 'Preview') + '</button></a>');
            ...
            Event.add(ul, 'click', function (e) {
                ...
                var item = DOM.getParent(e.target, 'li.nav-item');

                if (DOM.hasClass(item, 'active')) {
                    return;
                }

                DOM.removeClass(DOM.select('li, button', ul), 'active');
                DOM.setAttrib(DOM.select('button', ul), 'aria-expanded', 'false');

                makeTabActive(item);

                // get the action from tab button
                var action = DOM.getAttrib(DOM.select('button[aria-controls]', item)[0], 'aria-controls');
    ...
    // Init WfEditor with passed in setting object
    document.addEventListener('DOMContentLoaded', function handler() {
        WfEditor.init(WfEditorSettings || {});
        // reset
