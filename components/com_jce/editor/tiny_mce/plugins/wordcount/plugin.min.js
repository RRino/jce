(function (tinymce) {
  ...
  const classify = function (string) {
    ...
    return tinymce.map(string.split(''), memoized);
  };

  var ci = characterIndices;
  ...
  const getWords = function (string, options) {
  ...
  var getCount = function (editor) {
    return getWords(getTextContent(editor)).length;
  };

  const get = function (editor) {
    var getCount$1 = function () {
      return getCount(editor);
    };
  ...
  const setup = function (editor) {
    var Delay = tinymce.util.Delay, DOM = tinymce.DOM;

    var wordsToText = function (editor) {
      return getCount(editor).toString();
    };

    editor.onWordCount = new tinymce.util.Dispatcher(self);

    var limit = parseInt(editor.getParam('wordcount_limit', 0)), showAlert = editor.getParam('wordcount_alert', 0);

    var update = function () {
      var total = wordsToText(editor), amt = total;
      ...
      if (limit) {
        ...
        if (amt < 0) {
          DOM.addClass('wordcount', 'mceWordCountLimit');

          if (showAlert) {
            editor.windowManager.alert(editor.getLang('wordcount.limit_alert', 'You have reached the word limit set for this content.'));
          }

        } else {
          DOM.removeClass('wordcount', 'mceWordCountLimit');
        }
      }

      editor.onWordCount.dispatch(editor, amt);
      DOM.setHTML(editor.id + '_word_count', total);
    };
    ...
    editor.onInit.add(function () {
      ...
      row = DOM.get(editor.id + '_path_row');

      if (row) {
        Delay.setEditorTimeout(editor, function () {
          DOM.add(row.parentNode, 'div', {
            'class': 'mceWordCount'
          }, editor.getLang('wordcount.words', 'Words: ') + '<span id="' + editor.id + '_word_count" class="mceText">0</span>');

          editor.selection.onSetContent.add(debouncedUpdate);
          editor.onSetContent.add(debouncedUpdate);
          editor.onUndo.add(debouncedUpdate);
          editor.onRedo.add(debouncedUpdate);
          editor.onKeyUp.add(debouncedUpdate);
  ...
  tinymce__default['default'].PluginManager.add('wordcount', function (editor) {
    setup(editor);
    return get(editor);
  });
